<x-app-layout>
    <div class="container lg:w-3/4 md:w-4/5 w-11/12 mx-auto my-8 px-8 py-4 bg-white shadow-md">
        ...
        <article class="mb-2">
            ...
            <img src="{{ $post->image_url }}" alt="" class="w-80 mb-2">
            <p class="text-gray-700 text-base">{!! nl2br(e($post->body)) !!}</p>
        </article>
        <div class="flex flex-row w-full">
            <div class="flex flex-row text-center my-4 mr-auto ml-0">
                @can('update', $post)
                    <a href="{{ route('posts.edit', $post) }}"
                        class="bg-green-400 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-20 mr-2">編集</a>
                @endcan
                @can('delete', $post)
                    <form action="{{ route('posts.destroy', $post) }}" method="post">
                        ...
                        <input type="submit" value="削除" onclick="if(!confirm('削除しますか？')){return false};"
                            class="bg-red-400 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-20">
                    </form>
                @endcan
            </div>
                ...
                @else
                    <form action="{{ route('posts.nices.destroy', [$post, $my_nice]) }}" method="POST">
                        ...
                        <input type="submit" value="取り消し"
                            class="bg-green-500 hover:bg-gray-300 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                    </form>
                @endif
            </div>
        </div>
